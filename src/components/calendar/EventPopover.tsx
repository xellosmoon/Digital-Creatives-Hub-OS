...
export default function EventPopover({
  ...
  const getCategoryStyles = () => {
    switch (categoryName) {
      case 'Tech & Dev':
        return 'bg-cyan-500/10 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-300 border-cyan-300/50 dark:border-cyan-700/50';
      case 'Workshops':
        return 'bg-amber-500/10 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border-amber-300/50 dark:border-amber-700/50';
      case 'Community':
        return 'bg-emerald-500/10 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 border-emerald-300/50 dark:border-emerald-700/50';
      default:
        return 'bg-blue-500/10 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 border-blue-300/50 dark:border-blue-700/50';
    }
  };
  ...
  return (
    ...
    >
      <div 
        className="relative w-full max-w-md bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl border border-slate-200/80 dark:border-slate-700/80 shadow-2xl rounded-2xl p-6 transition-all"
        onClick={(e) => e.stopPropagation()} // prevent clicks inside popover from closing it
      >
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 rounded-full hover:bg-slate-100/80 dark:hover:bg-slate-700/80 transition-colors text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"
          aria-label="Close"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="space-y-3 mb-6 text-sm text-slate-600 dark:text-slate-400">
          ...
          {(event.organization || event.organizer) && (
            <div className="flex items-center gap-3">
              ...
              <p>Hosted by <span className="font-medium text-slate-900 dark:text-white">{event.organization || event.organizer}</span></p>
            </div>
          )}

          {/* Description */}
          {event.description && (
            <div className="mt-4 p-3 rounded-xl bg-slate-50/50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-600">
